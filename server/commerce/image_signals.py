...
def create_order_summary_image(purchase):
    ...
    width = 306  # 4.25" at 72 DPI
    height = 936  # 13" at 72 DPI
    ...
    draw.text((padding + 30, y_position), "Description", fill=BLACK, font=normal_font)
    y_position += 20
    ...
    for item in items:
        ...
        qty_x = padding + (30 - qty_width) // 2

        draw.text((qty_x, y_position), qty_text, fill=GRAY, font=normal_font)

        description_lines = wrap_text(description, normal_font, width - padding - 80)
        for line in description_lines:
            draw.text((padding + 30, y_position), line, fill=GRAY, font=normal_font)
            y_position += 18
