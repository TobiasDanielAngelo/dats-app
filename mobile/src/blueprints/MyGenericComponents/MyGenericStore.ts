...
export async function guidedRequest<T>(
  ...
  let url = customURL ? `${customURL}/${endpoint}` : `${BASE_URL}/${endpoint}`;
...
>(
  ...
  const props = {
    ...
    pageDetails: prop<PaginatedDetails>(() => defaultPageDetails),
  ...
  class GenericStore extends Model(props) {
    ...
    fetchAll = _async(function* (
      ...
      if (!temporary) {
        ...
        this.latestParam = params?.toString() ?? "page=1";
    ...
    fetchTemp = _async(function* (this: GenericStore, params?: string) {
      return this.fetchAll(params?.toString(), true);
    });
  ...
  return GenericStore as unknown as new (args: any) => Methods &
    GenericInterface &
